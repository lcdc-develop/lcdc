...
class TestSplitByGaps(unittest.TestCase):
    ...
    def test_call(self):
        ...
        record[TC.DATA] = np.zeros((len(t),5))
        record[TC.DATA][:,0] = t
        record[TC.PERIOD] = 2
        ...
        assert np.all(result[0][TC.DATA][:,0] == t[:4]), f"Wrong split: Expected {t[:4]}, got {result[0][TC.DATA][:,0]}"
        assert np.all(result[1][TC.DATA][:,0] == t[4:]-t[4]), f"Wrong split: Expected {t[:4]}, got {result[0][TC.DATA][:,0]}"
...
class TestSplitBySize(unittest.TestCase):
    ...
    def test_basic(self):
        ...
        for r, s in zip(result, [2,2,2,2]):
            ...
            assert len(r[TC.DATA]) == s, f"Wrong length: Expected {s}, got {len(r[TC.DATA])}"
            start += s
